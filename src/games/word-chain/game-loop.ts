...
import { fetchDatamuseResult } from "../../datamuse/api";
import { shuffleArray } from "../../utils/array";
import sleep from "../../utils/sleep";

const logger = pino({ prettyPrint: process.env.NODE_ENV !== "production" });

export const changeTurnForPlayerVsAi = async (
  message: Message,
  timeLeft?: number,
) => {
  ...
  const eliminatePlayer = async () => {
    ...
    await changeTurnForPlayerVsAi(message);
  };
  ...
  const turnSeconds: number[] = easyTurnSeconds;
  ...
  if (currentGame.userIds.length === 1) {
    ...
    const embed = new MessageEmbed()
      ...
      .addField("Score", `${currentGame.score[winnerUserId]}`)
  ...
  if (!timeLeft) {
    const embed1 = new MessageEmbed()
      ...
      .addField("Score", `**${currentGame.score[currentGame.currentUser]}**`);
  ...
  if (currentGame.currentUser === message.client.user!.id) {
    ...
    const response = await fetchDatamuseResult({
      StartLetter: {
        kind: "stringValue",
        stringValue: currentGame.currentStartingLetter,
      },
      number: {
        numberValue: currentGame.currentWordMinLength,
      },
    });

    if (!(response instanceof Array)) {
      message.channel
        .send({ content: "API error occurred. Ending the game." })
        .catch((e) => {
          // eslint-disable-next-line no-console
          console.error(e);
        });
    ...
    const wordForBot = response
      .map((r) => r.word)
      .filter((word) => !currentGame?.usedWords.includes(word))[0];

    if (
      wordForBot &&
      wordForBot[0].toLowerCase() ===
        currentGame.currentStartingLetter.toLowerCase() &&
      wordForBot.length >= currentGame.currentWordMinLength
    ) {
      const word = wordForBot;

      message.channel.send({ content: `**${word}**` }).catch((e) => {
        // eslint-disable-next-line no-console
        console.error(e);
      });

      const usedWords = [
        ...activeGames[channelId]!.usedWords,
        word.toLowerCase(),
      ];
      ...
      if (longestWord === word) {
        longestWordUserId = message.client.user!.id;
      }
      ...
      activeGames[channelId] = {
        ...activeGames[channelId]!,
        currentUser: currentGame.userIds.find(
          (u) => u !== currentGame?.currentUser,
        )!,
        longestWord,
        longestWordUserId,
        usedWords,
        ...
        reduce: shouldReduce,
        score: {
          ...activeGames[channelId]!.score,
          [currentGame.currentUser]:
            activeGames[channelId]!.score[currentGame.currentUser] + 1,
        },
      };
    ...
    const word = currentPlayerMessage.content
      .split(" ")[0]
      .replace(/[^\w]/g, "");
    ...
    if (condition1 || condition2 || condition3 || condition4 || condition5) {
      ...
      await changeTurnForPlayerVsAi(message, newTimeLeft);
